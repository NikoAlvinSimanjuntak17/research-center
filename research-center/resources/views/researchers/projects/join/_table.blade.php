        ...
        <tbody>
            @foreach ($projects as $project)
                <tr>
                    ...
                    <td>
                        <button type="button" class="btn btn-info btn-sm" data-bs-toggle="modal" data-bs-target="#projectDetailModal{{ $project->id }}">
                            Detail
                        </button>

                        @if ($canJoin && $project->status === 'open')
                            <form action="{{ route('researchers.projects.join', $project->id) }}" method="POST" class="d-inline">
                                @csrf
                                <button class="btn btn-success btn-sm" onclick="return confirm('Yakin ingin bergabung?')">Join</button>
                            </form>
                        @endif
                    </td>
                </tr>
