        ...
        <div class="card shadow mb-4">
            <div class="card-body">
                <dl class="row">
                    ...
                    <dd class="col-sm-8">{{ $collaborator->department->name ?? '-' }}</dd>
                    ...
                    <dd class="col-sm-8">
                        {{ $collaborator->created_at ? $collaborator->created_at->format('d M Y H:i') : '-' }}</dd>
                </dl>
            </div>
        </div>

        <a href="{{ route('researcher.projects.collaborators.index', ['id' => $project->id]) }}" class="btn btn-secondary">
            ← Kembali ke Daftar
        </a>
    </div>
@endsection